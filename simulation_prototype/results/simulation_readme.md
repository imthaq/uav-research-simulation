...
simulation_prototype/
  ...
  simulation_visualizer.py   - replays a scenario's CSV log as an animated view, exports MP4/GIF, live view
  ...
  media/                     - MP4s generated by simulation_visualizer.py (one per scenario, plus a fusion-mode comparison)
```
...
Generate/replay videos from the CSV logs (see "Visualizing runs" below for full options):
...
## Visualizing runs (simulation_visualizer.py)

Replays any scenario's CSV log as an animated matplotlib view and/or exports
it as a video. No arguments needed for the common case:

    python simulation_visualizer.py

With no `--log` given, this runs **batch mode**: it reads the scenario list
straight out of `simulation_config.json`, loads each scenario's `run1` log
from `logs/`, pops up an auto-playing preview window for each one (skipped
automatically if there's no display), and saves an MP4 per scenario to
`media/` (skipped if `ffmpeg` isn't on PATH). It also generates a side-by-side
**fusion-mode comparison video** (`media/fusion_comparison_video.mp4`) showing
`naive_fusion`, `trust_weighted_fusion`, and `no_fusion_matched` in synced
panels, as long as at least two of those scenarios have logs available.

Useful flags for batch mode: `--no-popup` (skip preview windows, just save
MP4s), `--logs-dir` / `--media-dir` (override default folders), `--fps`.

To work with a single log instead of the whole batch, pass `--log`:
    ...
    python simulation_visualizer.py --log logs/baseline_run1.csv --mode gif --output media/baseline_video.gif

`--mode` options:
* `interactive` (default) - opens a window; use Left/Right arrow keys to step through frames, `q` to quit.
* `mp4` / `gif` - exports the full replay to `--output` (path extension must match the mode).
* `live-demo` - drives the log through the same `update_step()` call pattern a *running* simulation would use (see `LiveSimulationView` below), rather than the static replay path. Mainly useful for testing that integration without editing `simple_swarm_sim.py`.

To generate only the fusion comparison video (skip everything else):

    python simulation_visualizer.py --fusion-comparison

### What's drawn on screen

Each frame maps directly onto the CSV columns described above:

| CSV column(s) | Drawn as |
|---|---|
| `uav_pos_x`, `uav_pos_y` | Colored dot per UAV, labeled `U{uav_id}` |
| (accumulated `uav_pos_x/y` over time) | Faint trailing line = that UAV's path so far |
| `goal_pos_x`, `goal_pos_y` | Hollow square in the UAV's color |
| `actual_obstacle_x`, `actual_obstacle_y` | Solid red circle (ground truth, static) |
| `perceived_obstacle_x`, `perceived_obstacle_y` | Dashed circle in the UAV's color (blank if not currently detected) |
| `collision_risk_flag` + `nearest_entity_distance` | Translucent orange collision-risk zone around the UAV |
| `scenario`, `step`, `time_s`, `action_taken`, `perception_error_type` | Info box (scenario name, current time step, current action, current error type) |
| `mission_completed_flag` (across the whole log) | Mission status line in the info box - see below |

### Mission SUCCESS / FAILURE

The info box's `Mission:` line is not just an instant readout of that one
row's `mission_completed_flag` - the visualizer scans the entire log up
front (`SimulationData.from_csv`) to find the first step, if any, where
`mission_completed_flag` goes `True`:

* **SUCCESS** (green box) - shown from that step onward.
* **FAILURE** (red box) - shown once playback reaches the log's final step
  without `mission_completed_flag` ever having gone `True` (e.g. every
  `trust_weighted_fusion` run, which never completes a mission successfully).
* **In Progress** (tan box) - any earlier step of a run that will go on to succeed.

### Live view (LiveSimulationView)

`simulation_visualizer.py` also exposes a `LiveSimulationView` class meant
to be called *from inside a running simulation's step loop* - i.e. live, not
a replay of an already-finished CSV. `simple_swarm_sim.py` does not call
this yet; wiring it in is a small follow-up (see `initial_results_summary.md`,
"What will be improved next"). Usage once wired in:

    from simulation_visualizer import LiveSimulationView

    view = LiveSimulationView(scenario_name="baseline", world_width=100, world_height=100,
                               obstacle_pos=(50, 50))
    for step in range(num_steps):
        rows = [uav.to_log_row(step) for uav in uavs]  # one dict per UAV, same fields as a CSV row
        view.update_step(step, rows)
    view.close()

If no display is available (e.g. running headless on a server), `update_step`
disables itself silently after the first failed attempt so a simulation run
is never blocked by the lack of a GUI.
