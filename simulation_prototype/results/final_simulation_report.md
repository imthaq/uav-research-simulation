...
**Config hash basis:** `simulation_config.json` (62 scenarios, 4-UAV swarm)
...
62 scenarios total, defined in `simulation_config.json`, grouped by what
they exercise:
...
- **Final result package (`generate_final_result_package.py`, Task 13):**
  built into `results/final/` at **62 scenarios × 20 trials = 1240 runs**
...
| File | Rows / scope |
|---|---|
| `raw_run_index.csv` | 1240 rows (62 scenarios × 20 trials) |
| `scenario_summary.csv` | 62 rows (one per scenario) |
...
- **No vision/lidar results in this package:** `models/vision_like_model.py`
  ...
  described here. Although fully implemented (including latency and dropout),
  they remain intentionally outside the frozen 62-scenario matrix.
- **Handoff Mechanism:** A safe-fallback handoff mechanism has been implemented
  in `dependability/perception_handoff_model.py`, augmenting swarm decision robustness.
